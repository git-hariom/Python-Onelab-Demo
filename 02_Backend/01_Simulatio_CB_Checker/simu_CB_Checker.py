...
reportFile_path = r"C:\Users\Public\report.txt"
...
def TestCase_CheckforOUT(var_Name,var_DataType):
    ...
    if var_Name.casefold() in var_dict_in_ucDef:
        ...
        if (var_DataType.casefold() == 'bool' or var_DataType.casefold() == 'word') and var_dict_in_ucDef[var_Name.casefold()][1].casefold() == 'int':
            MakeReport("MDD Output: '" + var_Name + "' -> OK\n")
        else:
            MakeReport("MDD Output: '" + var_Name + "' -> DataType Not matching\n")

    # TestCase 4
    if var_Name.casefold() in var_dict_out_ucDef:
        MakeReport("MDD Output: '" + var_Name + "' is as a output in ucDef and MDD\n")

    # TestCase 7
    if var_Name.casefold() not in var_dict_in_ucDef and var_Name.casefold() not in var_dict_out_ucDef:
        MakeReport("MDD Output: '" + var_Name + "' not present in ucDef\n")
...
def RemainingVars_ucDef():
    for i in var_dict_out_ucDef:
        MakeReport("ucDef Output: '" + var_dict_out_ucDef[i][0] + "' not present in MDD\n")
    for i in var_dict_in_ucDef:
        MakeReport("ucDef Output: '" + var_dict_in_ucDef[i][0] + "' not present in MDD\n")

def MakeReport(msg):
    global report_msg
    report_msg += str(msg)

def StartChecker(ucDef_path, MDD_path):
